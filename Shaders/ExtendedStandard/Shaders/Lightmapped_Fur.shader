...
Shader "Extended Standard/Lit/Fur" {
	Properties {
    ...
    inline half4 getVertexGI(float2 uv1, float2 uv2, float3 posWS, float3 nDirWS )
    {
		...
		v2f vert_internal(appdata_t v, half FUR_OFFSET)
		{
			...
			o.lightmapUV = getVertexGI(v.uv1, v.uv2, o.posWorld, o.normalDir);
		...
		fixed4 frag_internal(v2f i, half FUR_OFFSET)
		{
			...
			fixed alpha = _BaseColor.a;//tex2D(_LayerTex, TRANSFORM_TEX(i.texcoord.xy, _LayerTex)).r;
			_BaseColor.a = 1.0;

      float alf = lerp(_Cutoff, _CutoffEnd, -FUR_OFFSET);
      alf = clamp(pow(alf, 0.5), 0.0, 1.0);
			alpha = step(alf, alpha);
